...
Live Production QA succeeded after `Add_Pro_Forma.Sale_Price_FF` was published with **Max Digits 14, Decimal Points 2**. With Max Digits 10 and Decimal Points 2, Creator had still rejected the report update on max digits. Test Pro Forma `4410926000004947002` saved a blended `Sale_Price_FF` of `1444.45` from a `1500.01`/LF Lot Mix price. Its dashboard showed Gross Sales **$8,088,630**, Net Profit **$6,658,630**, 47 saved month rows, and January 2030 completion. After locking, Lot Sales phase cards remained navigable while inputs stayed disabled. These observations verify the combined live configuration and widget path; they do not isolate which change removed the original validation error. No Creator function, workflow, or Custom API was changed for this release. Regression: phase timing, explicit Esc dates, price breakdown, legacy cash flows, fractional save, and locked navigation. Widget rollback: map Development and Production to `1.80.39`; Creator field precision is managed separately.

## 2026-09-25 — Pro Forma two-decimal save trigger (1.80.40)

The Pro Forma server-engine header touch tried the SDK's direct-data shape after a `Sale_Price_FF` zero-decimal rejection. Live Production testing showed that Creator rejects the direct shape with `2945 EXTRA_KEY_FOUND_IN_JSON`, while the documented nested shape still returned `3002` for a fractional price. The test Pro Forma was restored to its original pricing, saved with verified month/financial parity, and relocked. Development and Production were rolled back to `1.80.39`; the `1.80.40` release remains an immutable record of the failed experiment. No Creator form, field, function, or Custom API source changed in that release. The later Creator precision change and successful fractional save are documented in `1.80.41` above. Rollback release: `1.80.39`.
...
## Comments 1.79.6 / Contracts 1.60.16 / Budget Manager 122.27.2

- Legal and Budget row controls are now matched 25px buttons. Legal attachments use the same visual state rules and count badge as comments, and both modal close icons are explicitly drawn.
- Budget's shared comment thread now resolves the active `Project` or `Budget` field for every read and write. Switching from a Project thread to a Budget thread can no longer issue a `Project == budgetId` query and hide comments that the count badge has found.
...
## 2026-09-15 — Pro Forma comment modal refinements (1.79.1)

Placed one larger comment icon immediately before the record navigation so it remains available across all record tabs. Simplified the modal header, moved editing into the selected message with Save and Cancel, removed search and reload controls, revised the action controls, and raised confirmations over the modal. Rollback: Proforma Manager 1.79.0.
