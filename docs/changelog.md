...
## 2026-09-25 — Pro Forma phase pricing display and decimal save diagnostic (1.80.41)

Lot Sales pairs project month numbers with calendar month/year, defaults an untouched Esc Start Date to the month after that phase's first sale, and calls the one-time pricing control Phase Increase. The phase schedule shows the final sale's per-lot base, increase, escalator, finished price, growth from the first sale, and the increase that would carry that finished price into the next phase. Dashboard Cash Flow and Inflows show Base Price, Phase Increase, and Escalator as detail beneath Finished Lot Sales without adding them again to Total Income. For fractional `Sale_Price_FF`, the workflow-triggering report update now supplies the exact two-decimal value and tries the second confirmed Pro Forma report only on Creator's specific zero-decimal rejection. This last behavior requires live Creator verification; it does not round the sale price or bypass server month/phase parity. Widget only; no Creator form, field, function, or Custom API changed. Regression: phase timing, explicit Esc dates, price breakdown, legacy cash flows, fractional save, and locked navigation. Rollback: map Development and Production to `1.80.39`.
