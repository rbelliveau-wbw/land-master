...
This widget release does not publish Creator functions or workflows. A Production Creator API that does not report phase-sales support still shows the legacy duplicate path until the phase-sales v2 workflow and `proforma_save` are published there. Regression: typing existing Lot Mix rows, explicit Add and blank-row reuse, PID/MUD Add and edit styling, draft title/date updates, new-tab progression, lot-count changes before/after manual phase edits, legacy/persisted duplicates, shared-settings confirm/cancel and locked later-phase controls, and unsupported Creator fallback. Rollback: map Production to widget `1.80.24`; Creator records are unchanged by this release.
...
The save first writes the old-calculation header, then phase inputs, triggers the server workflow, verifies month count, lots, sales dollars, and phase timing against the widget, and only then asks `proforma_save` to clear `Initial_Takedown` and `Lots_per_Month`. `Lot_Sales_Schedule_Version = 2` remains the persisted marker. If validation or recalculation fails, the old fields remain for recovery. The approval PDF reads phase-1 take values (and notes varying later phases) after conversion. The Excel Inputs sheet and dashboard similarly show phase values instead of blank legacy fields. This does not guarantee identical old and new financials: the phase engine rounds each take to whole dollars and fixes an old one-take later-phase receipt gap.

The hosted widget probes the environment's own `Save_PF`/`Save_PF1` for the new read-only `phase_sales_capabilities` operation. Until Creator is deployed, legacy entry and saves remain available; after deployment the phase editor activates on reload. Creator deployment is reserved for the user: install the phase-sales v2 workflow first, then update `proforma_save` and `PF_Build_Proforma_Approval_PDF` in each target environment. No batch migration should run. The phase navigation cards are visibly clickable, and a corrected closing tag makes the schedule a direct right-hand grid item on desktop. Regression: legacy open without write, round-up allocation, DEV/Production capability isolation, legacy fallback before Creator deployment, verified finalization and old-field clearing, month/phase parity, phase-card keyboard focus, three-column DOM structure, dashboard/Excel/PDF post-conversion values, and phone stacking. Rollback: remap Development and Production to widget `1.80.21`; restore the prior Creator function bodies only after assessing any records already converted.
...
Lot Sales is a separate editor tab immediately after Project Schedule. Its phase cards use a compact, capped-width layout with a schedule timeline; Monthly/Quarterly and the shared-settings/escalator switches use pill controls. Project Start and Esc Start Date use a custom month/year picker. A newly selected month saves as its first day. A blank Esc Start Date defaults to the month after that phase's first sale, and an auto-filled date follows schedule changes until the user explicitly selects another month. Existing saved dates are not silently changed. Month numbers in the Lot Sales schedule are paired with calendar month/year. The one-time markup is labeled Phase Increase in the UI; persisted field names remain unchanged. The summary shows final-sale per-lot base, phase increase, escalator, finished price, growth from the first sale, and a suggested next-phase increase to carry the final price. Dashboard Cash Flow and Inflows show base, phase increase, and escalator as detail beneath Finished Lot Sales without changing totals. The Months & Phases preview and the existing Excel export show phase-sale inputs and the six monthly pricing/escalator components; new columns stay blank on legacy records.
