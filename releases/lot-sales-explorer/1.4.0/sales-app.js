(function () {
  ...
  function selectedFilters() { return { projectIds: InsightsControls.values($('project')), territories: InsightsControls.values($('territory')), builderIds: InsightsControls.values($('builder')),
    search: $('search').value.trim(), statuses: InsightsControls.values($('status')), dateField: $('dateField').value, excludeBuilders: $('excludeBuilders').checked,
  ...
  function render() {
    ...
    $('summary').innerHTML = '<div class="summary-groups">' + r.reports.map(part => '<div class="status-summary"><strong>' + esc(part.status) + '</strong><span><b>' + integer(part.stats.count) + '</b> lots</span><span><b>' + integer(part.rows.length) + '</b> subdivisions</span><span><b>' + valueLabel(part.stats, 'avgPriceFF') + '</b> avg base $/FF</span><span><b>' + money(part.stats.totalPrice) + '</b> base price</span></div>').join('') + '</div><span class="summary-scope">' + esc(dateName.toUpperCase()) + ' · NEWEST FIRST</span>';
    ...
    const territories = new Map(), allCounts = state.historyReady ? M.subdivisionCounts(state.lots) : null;
    r.rows.forEach(row => { const key = JSON.stringify([row.status,row.territory]); if (!territories.has(key)) territories.set(key, []); territories.get(key).push(row); });
    ...
    territories.forEach((rows, territory) => {
      ...
      html += '<tr class="territory"><th scope="rowgroup"><button class="territory-toggle" data-territory="' + esc(territory) + '" aria-expanded="' + !closed + '"><span aria-hidden="true">' + (closed ? '▸' : '▾') + '</span>' + esc((r.statuses.length > 1 ? rows[0].status + ' · ' : '') + rows[0].territory) + '</button></th><th colspan="' + (months.length + 3) + '">' + rows.length + ' subdivisions · ' + integer(count) + ' lots</th></tr>';
      if (!closed) rows.forEach(row => {
        const counts = allCounts?.get(row.id), countText = counts ? 'Total ' + integer(counts.total) + ' · Sold ' + integer(counts.sold) + ' · Contracted ' + integer(counts.contracted) + ' · Open ' + integer(counts.open) : state.historyError ? 'All-lot counts unavailable' : 'All-lot counts loading';
        const soldOut = counts && counts.total > 0 && counts.sold === counts.total;
        const badge = soldOut ? '<span class="sold-out-badge" title="Every lot sold" aria-label="Sold out"><svg viewBox="0 0 20 20" aria-hidden="true"><path d="M4 10.5 8 14l8-8"/></svg></span>' : '';
        html += '<tr><td class="sticky" title="' + esc(row.project) + '"><span class="subdivision-counts" tabindex="0" title="' + esc(countText) + '" aria-label="' + esc(row.name + '. ' + countText) + '">' + esc(row.name) + '</span>' + badge + (r.statuses.length > 1 ? '<small class="status-tag">' + esc(row.status) + '</small>' : '') + '</td><td class="identity">' + (state.historyReady ? dateLabel(row.firstSale) : state.historyError ? 'Unavailable' : 'Loading…') + '</td><td class="identity">' + decimal(row.stats.avgWidth) + '</td>' + cell(row.stats, row, '', true) + months.map(month => cell(row.cells.get(month), row, month, false)).join('') + '</tr>';
      });
    });
  ...
  function exportMatrix() {
    ...
    const rows = [['Land Master Insights', M.metrics[metric].label], ['Status', r.status, 'Date basis', r.dateField === 'closeDate' ? 'Close Date' : 'Purchase Date'], ['From', r.from, 'To', r.to],
      ['Status', 'Territory', 'Subdivision', 'Project', 'First lot sale (all dates)', 'Average front ft (selected period)', 'Period ' + M.metrics[metric].label, ...r.months]];
    r.rows.forEach(row => rows.push([row.status, row.territory, row.name, row.project, row.firstSale, row.stats.avgWidth, value(row.stats), ...r.months.map(month => value(row.cells.get(month)))]));
  ...
  $('collapse').addEventListener('click', () => { const all = [...new Set((state.report && state.report.rows || []).map(r => JSON.stringify([r.status,r.territory])))]; if (all.every(t => state.collapsed.has(t))) state.collapsed.clear(); else all.forEach(t => state.collapsed.add(t)); render(); });
  ...
  $('reset').addEventListener('click', () => { ['territory','project','builder'].forEach(id => InsightsControls.setValues($(id),[])); $('search').value = ''; InsightsControls.setValues($('status'),['Sold']); $('dateField').value = 'closeDate'; $('period').value = 'twoYears'; $('excludeBuilders').checked = true; $('metric').value = 'avgPriceFF'; $('sort').value = 'name'; state.collapsed.clear(); applyPeriod(); update(); });
  $('audit').addEventListener('click', () => { $('auditText').textContent = 'Land Master Insights v1.4.0\n' + JSON.stringify(LMRuntime.current(), null, 2) + '\n\n' + state.log.join('\n'); $('diagnostics').showModal(); });
