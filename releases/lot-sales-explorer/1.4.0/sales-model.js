...
(function (root) {
  ...
  function reportSelection(rows, filters) {
    ...
    return { reports, statuses, status:statuses.join(' & '), dateField:reports[0].dateField, from, to, months:monthRange(from,to),
      rows:reports.flatMap(r=>r.rows.map(row=>({...row,status:r.status,key:JSON.stringify([r.status,row.id])}))),
      lots:reports.flatMap(r=>r.lots), missingDates:reports.reduce((n,r)=>n+r.missingDates,0) };
  }
  root.LotSalesModel = Object.freeze({ text, id, numeric, date, normalize, totalPrice, subdivisionCounts, stats, report, reportSelection, monthRange, metrics: METRICS, csv });
