(function () {
  ...
  function position() {
    ...
    const width = Math.min(innerWidth - 16, Math.max(rect.width, opened.month ? 286 : opened.plain ? 210 : 290));
  ...
  function mountSelect(control) {
    const label = labelOf(control), multiple = control.multiple, plain = control.dataset.picker === 'single';
    ...
    function open() {
      ...
      function draw(focusValue) {
        ...
        list.innerHTML = visible.length ? visible.map(o=>'<button type="button" class="picker-option'+(o.selected?' selected':'')+'" role="option" aria-selected="'+o.selected+'" data-value="'+esc(o.value)+'">'+(plain?'':'<span class="picker-check" aria-hidden="true">'+(o.selected?'✓':'')+'</span>')+'<span>'+esc(o.text)+'</span></button>').join('') : '<div class="picker-no-results">No matches</div>';
  ...
  window.InsightsControls=Object.freeze({values,setValues,syncAll:()=>{entries.forEach(e=>e.sync());syncSearch();},close});
})();
