(function () {
  ...
  function render() {
    ...
    $('summary').innerHTML = '<div class="summary-groups">' + r.reports.map(part => '<div class="status-summary"><strong>' + esc(part.status) + '</strong><span><b>' + integer(part.stats.count) + '</b> lots</span><span><b>' + integer(part.rows.length) + '</b> subdivisions</span><span><b>' + valueLabel(part.stats, 'avgPriceFF') + '</b> avg $/FF</span><span><b>' + money(part.stats.totalPrice) + '</b> base price</span></div>').join('') + '</div><span class="summary-scope">' + esc(dateName.toUpperCase()) + ' · NEWEST FIRST</span>';
    ...
    $('quality').textContent = [r.reports.some(part=>part.stats.missingPriceFF) ? integer(r.reports.reduce((n,part)=>n+part.stats.missingPriceFF,0)) + ' lots without usable $/FF' : '', state.historyReady && r.missingDates ? integer(r.missingDates) + ' lots missing ' + dateName.toLowerCase() : ''].filter(Boolean).join(' · ');
    ...
    const territories = new Map(); r.rows.forEach(row => { const key = JSON.stringify([row.status,row.territory]); if (!territories.has(key)) territories.set(key, []); territories.get(key).push(row); });
    ...
    territories.forEach((rows, territory) => {
      ...
      if (!closed) rows.forEach(row => { html += '<tr><td class="sticky" title="' + esc(row.project) + '">' + esc(row.name) + (r.statuses.length > 1 ? '<small class="status-tag">' + esc(row.status) + '</small>' : '') + '</td><td class="identity">' + (state.historyReady ? dateLabel(row.firstSale) : state.historyError ? 'Unavailable' : 'Loading…') + '</td><td class="identity">' + decimal(row.stats.avgWidth) + '</td>' + cell(row.stats, row, '', true) + months.map(month => cell(row.cells.get(month), row, month, false)).join('') + '</tr>'; });
    });
  ...
  $('audit').addEventListener('click', () => { $('auditText').textContent = 'Land Master Insights v1.2.0\n' + JSON.stringify(LMRuntime.current(), null, 2) + '\n\n' + state.log.join('\n'); $('diagnostics').showModal(); });
