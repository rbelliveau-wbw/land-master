(function () {
  ...
  function placeMetricPicker() {
    const slot = $('metricSlot');
    if (!slot) return;
    if (matchMedia('(max-width: 700px)').matches) {
      slot.textContent = M.metrics[$('metric').value].label;
      InsightsControls.placePicker('metric', $('metricMobileSlot'));
    } else {
      slot.textContent = '';
      InsightsControls.placePicker('metric', slot);
    }
  }
  ...
  function render() {
    ...
    $('matrixHead').innerHTML = '<tr><th class="sticky" scope="col"><span class="sales-header-caption">Subdivision</span><span id="groupBySlot" class="sales-header-picker"></span></th><th scope="col" title="Earliest Sold close date, across all dates, within current scope filters">First Lot Sale<small>All Dates</small></th><th scope="col" title="Mean positive front footage for lots in this period">Avg Front Ft<small>Selected Period</small></th><th scope="col" class="period-total" title="Calculated over the entire selected period, including offscreen months"><span class="sales-header-caption">Period</span><span id="metricSlot" class="sales-header-picker"></span></th>' + months.map(m => '<th scope="col">' + esc(monthLabel(m).split(' ')[0]) + '<small>' + m.slice(0, 4) + '</small></th>').join('') + '</tr>';
  ...
  ['metric','sort'].forEach(id => $(id).addEventListener('change', render));
  ...
  window.addEventListener('resize', placeMetricPicker);
  ...
  function setPanel(toggleId, panelId, open) {
    ...
    if (toggleId === 'allFilters') $('salesDashboard').querySelector('.sales-filters').classList.toggle('is-expanded', open);
  }
  [['allFilters', 'moreFilters'], ['tableViewToggle', 'tableViewPanel']].forEach(([toggleId, panelId]) => {
    $(toggleId).addEventListener('click', () => setPanel(toggleId, panelId, $(panelId).hidden));
  });
  ...
  $('audit').addEventListener('click', () => { $('auditText').textContent = 'Land Master Insights v1.5.5\n' + JSON.stringify(LMRuntime.current(), null, 2) + '\n\n' + state.log.join('\n'); $('diagnostics').showModal(); });
