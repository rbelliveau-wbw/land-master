(function () {
  ...
  function render() {
    ...
    $('monthPage').textContent = months.length ? monthLabel(months[0]) + ' – ' + monthLabel(months[months.length - 1]) + ' · ' + months.length + ' of ' + r.months.length + ' months' : 'No months';
    ...
    $('matrixHead').innerHTML = '<tr><th class="sticky" scope="col" aria-label="Group and subdivision name">Name<small>Grouped by ' + esc($('groupBy').selectedOptions[0]?.text || 'Project') + '</small></th><th scope="col" title="Earliest Sold close date, across all dates, within current scope filters">First Lot Sale<small>All dates</small></th><th scope="col" title="Mean positive front footage for lots in this period">Avg Front Ft<small>Selected range</small></th><th scope="col" class="period-total" title="Calculated over the entire selected range, including offscreen months">' + esc(def.label) + '<small>Selected range</small></th>' + months.map(m => '<th scope="col">' + esc(monthLabel(m).split(' ')[0]) + '<small>' + m.slice(0, 4) + '</small></th>').join('') + '</tr>';
  ...
  $('audit').addEventListener('click', () => { $('auditText').textContent = 'Land Master Insights v1.5.6\n' + JSON.stringify(LMRuntime.current(), null, 2) + '\n\n' + state.log.join('\n'); $('diagnostics').showModal(); });
