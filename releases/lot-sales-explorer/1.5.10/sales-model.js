...
(function (root) {
  ...
  const METRICS = {
    ...
    avgTotalPriceFF: { label: 'Average Base + Interest $/FF', kind: 'currency', description: 'Mean of each eligible lot’s (Base Price + Interest) ÷ front footage. Blank interest counts as zero; escalator percentage is shown separately.' },
    ...
    avgPriceWithInterest: { label: 'Average Base + Interest', kind: 'currency', description: 'Average recorded Base Price plus Interest across eligible lots. Blank interest counts as zero; lots without Base Price are excluded.' },
    ...
    totalPriceWithInterest: { label: 'Total Base + Interest', kind: 'currency', description: 'Sum of recorded Base Price plus Interest across eligible lots. Blank interest counts as zero; lots without Base Price are excluded.' }
  };
  ...
  function stats(rows) {
    ...
    const eligible = priced.filter(r => r.width !== null && r.width > 0), totalPriced = priced.filter(r => totalPrice(r) >= 0), totalEligible = totalPriced.filter(r => r.width !== null && r.width > 0), sum = (xs, fn) => xs.reduce((n, r) => n + fn(r), 0);
    return { count: rows.length, priced: priced.length, eligible: eligible.length, missingPriceFF: rows.length - eligible.length,
      ...
      avgPriceWithInterest: totalPriced.length ? sum(totalPriced, totalPrice) / totalPriced.length : null,
      totalPrice: priced.length ? sum(priced, r => r.price) : null,
      totalPriceWithInterest: totalPriced.length ? sum(totalPriced, totalPrice) : null,
