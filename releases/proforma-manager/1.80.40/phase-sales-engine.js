...
(function(root){
  ...
  function monthOf(value){
    ...
    if(a) return +a[1]>=1 && +a[1]<=12 ? {y:+a[2],m:+a[1]} : null;
  ...
  function plan(input){
    ...
    for(var i=0;i<count;i++){
      ...
      function emit(quantity){
        ...
        if(!Number.isFinite(elapsed) || !Number.isFinite(escalator) || !Number.isFinite(base+extra+escalator))
          throw Error(label+" has invalid escalator sales in month "+month+"; check its Esc Start Date.");
