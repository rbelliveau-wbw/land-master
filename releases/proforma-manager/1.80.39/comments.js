(function(root){
  ...
  function id(v){
    ...
    return v&&typeof v==='object'?String(v.ID||v.Id||v.id||v.value||v.Value||v.record_id||v.Record_ID||''):String(v||'');
  }
  ...
  function mount(el,api){
    ...
    function refresh(){var pf=active,field=parentField(),seq=++request;loading=true;error='';render();return api.load(pf).then(function(result){if(seq!==request||pf!==active)return;rows=(result||[]).filter(function(r){return id(r[field])===pf;}).sort(function(a,b){var t=timestamp(a.Added_Time)-timestamp(b.Added_Time);return t||String(a.ID).localeCompare(String(b.ID),undefined,{numeric:true});});if(typeof api.onRows==='function')api.onRows(pf,rows.slice());loading=false;status('');render();},function(err){if(seq!==request||pf!==active)return;loading=false;status(api.error(err)||'Could not load comments.',true);render();});}
  ...
  root.PFComments={mount:mount,identity:identity,author:author,canChange:canChange,timestamp:timestamp,markdown:markdown,color:color,id:id};
