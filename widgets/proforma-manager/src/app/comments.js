(function(root){
  ...
  function identity(v){return text(v).trim().toLowerCase().replace(/@zohocreator\.com$/,'');}
  ...
  function timestamp(v){
    ...
    var m=s.match(/^(\d{1,2})-([A-Za-z]{3})-(\d{4})[ T](\d{1,2}):(\d\d):(\d\d)$/);
    if(!m)return NaN;
    var month=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'].indexOf(m[2]);if(month<0)return NaN;
    var wall=Date.UTC(+m[3],month,+m[1],+m[4],+m[5],+m[6]),guess=wall;
  ...
  function canChange(row,current,now){var age=(now==null?Date.now():now)-timestamp(row.Added_Time);return !truth(row.Deleted)&&!!identity(current)&&identity(row.Added_User||row.User)===identity(current)&&Number.isFinite(age)&&age>=0&&age<DAY;}
