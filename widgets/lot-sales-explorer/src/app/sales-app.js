(function () {
  ...
  function exportMatrix() {
    ...
    const rows = [['Lot Sales Explorer', M.metrics[metric].label], ['Status', r.status, 'Date basis', r.dateField === 'closeDate' ? 'Close Date' : 'Purchase Date'], ['From', r.from, 'To', r.to],
  ...
  $('audit').addEventListener('click', () => { $('auditText').textContent = 'Lot Sales Explorer v1.0.0\n' + JSON.stringify(LMRuntime.current(), null, 2) + '\n\n' + state.log.join('\n'); $('diagnostics').showModal(); });
